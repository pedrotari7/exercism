...
describe('append entries to a list and return the new list', () => {
  ...
  it('non-empty mixed type lists', () => {
    const list1 = List.create('1', '2')
    const list2 = List.create(2, 3, 4, 5)
    expect(list1.append(list2)).toHaveValues('1', '2', 2, 3, 4, 5)
  })

  it('non-empty mixed type list with other mixed type lists', () => {
    const list1 = List.create('1', 2)
    const list2 = List.create('2', 3, '4', 5)
    expect(list1.append(list2)).toHaveValues('1', 2, '2', 3, '4', 5)
  })
...
describe('concat lists and lists of lists into new list', () => {
  ...
  it('list of lists with different types', () => {
    const list1 = List.create(1, 2)
    const list2 = List.create(3)
    const list3 = List.create()
    const list4 = List.create(4, 5, 6)
    const list5 = List.create('string')
    const listOfLists = List.create(list2, list3, list4, list5)
    expect(list1.concat(listOfLists)).toHaveValues(1, 2, 3, 4, 5, 6, 'string')
  })
})
...
describe('filter list returning only values that satisfy the filter function', () => {
  ...
  it('appended empty list', () => {
    const list1 = List.create()
    const list2 = List.create(1, 2, 3, 4, 5)
    const list3 = list1.append(list2)
    expect(list3.filter((el) => el % 2 === 1)).toHaveValues(1, 3, 5)
  })
})
...
describe('reverse the elements of a list', () => {
  ...
  it('list of lists is not flattened', () => {
    const list1 = List.create([1, 2], [3], [], [4, 5, 6], 7)
    expect(list1.reverse()).toHaveValues(7, [4, 5, 6], [], [3], [1, 2])
  })
})
